# src/fait/vision/services/object_detection_service.py
from __future__ import annotations
import logging
from dataclasses import dataclass
from typing import Optional, Dict, Tuple
from fait.core.paths import get_paths, ensure_on_first_write
from fait.vision.object_detection.models.grounding_dino import GroundingDINO, GDINOConfig
from fait.vision.object_detection.models.deformable_detr import DeformableDETR, DefDETRConfig
from fait.vision.object_detection.models.yolo import YOLODetector, YoloConfig

log = logging.getLogger("fait.vision.services.object_detection_service")
...
class ObjectModelsService:
    def __init__(self, cfg: ObjectServiceConfig = ObjectServiceConfig()):
        ...
        ensure_on_first_write(self.paths.models_object_detection)
    ...
    def get_gdino(self, cfg: Optional[GDINOConfig] = None) -> GroundingDINO:
        ...
        if key not in self._gdino_pool:
            self._gdino_pool[key] = GroundingDINO(c, cache_dir=str(self.paths.models_object_detection))
    ...
    def get_detr(self, cfg: Optional[DefDETRConfig] = None) -> DeformableDETR:
        ...
        if key not in self._detr_pool:
            self._detr_pool[key] = DeformableDETR(c, cache_dir=str(self.paths.models_object_detection))
    ...
    def get_yolo(self, cfg: Optional[YoloConfig] = None) -> YOLODetector:
        ...
        if key not in self._yolo_pool:
            self._yolo_pool[key] = YOLODetector(c, cache_dir=str(self.paths.models_object_detection))
