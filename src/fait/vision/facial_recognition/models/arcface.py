...
from fait.vision.services.facial_recognition_service import get_face_service, FaceService

log = logging.getLogger("fait.vision.speaker_recognition.arcface")

@register_embedder("arcface")
class ArcFaceEmbedder(Embedder):
    """
    ArcFace speaker_recognition via shared FaceService (no duplicate model load).
    """
    ...
    def mean_embedding(self, folder: str, use_cache: bool = True) -> np.ndarray:
        ...
        if not embs:
            raise ValueError("No valid reference speaker_recognition found.")
