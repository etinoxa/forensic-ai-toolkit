# src/fait/audio/speaker_recognition/models/wavlm_embedder.py
...
class WavLMEmbedder(_BaseAudioEmbedder):
    """
    Microsoft WavLM x-vector speaker models with safe chunking.
    ...
    def __init__(self, model_id: str = "microsoft/wavlm-base-plus-sv", cache_dir: Optional[str] = None,
        ...
        self.cache_dir = cache_dir or str(paths.models_speaker_recognition / "wavlm")
    ...
    def _forward_chunk(self, wav_chunk: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            out = self.model(**inputs)  # returns .models
    ...
    def _forward_single(self, wav: np.ndarray, max_len: int, *, return_tensor: bool = False) -> np.ndarray | torch.Tensor:
        ...
        out = self.model(**inputs)  # WavLMForXVector -> has .models (B, D)
