...
from fait.audio.services.speaker_service import get_speaker_service
from fait.audio.embeddings.base import AudioEmbedder
...
def run_speaker_match(cfg: SpeakerMatchConfig) -> Dict:
    t0 = time.time()
    paths = get_paths()
    base = Path(cfg.output_dir) if cfg.output_dir else (paths.outputs / "audio" / "speaker_match")
    ensure_folder(base)
