...
import fait.vision.ocr.models.paddle_engine as paddle_mod

class _FakePaddleOCR:
    def __init__(self, *a, **k): pass
    def ocr(self, img, det=True, rec=False, cls=False):
        # One page (outer list) with two plain polygons (inner list)
        # Shape: [ [ poly1, poly2 ] ], where each poly is [[x,y],...]
        if det and not rec:
            return [[
                [[1, 1], [60, 1], [60, 25], [1, 25]],
                [[2, 2], [40, 2], [40, 18], [2, 18]],
            ]]
        # If rec=True accidentally, return a benign old-format
...
def test_paddle_detect_contract(monkeypatch, tmp_paths):
    ...
    eng = paddle_mod.PaddleEngine(lang="en", cache_dir=str(tmp_paths.models_ocr / ".paddlex"))
